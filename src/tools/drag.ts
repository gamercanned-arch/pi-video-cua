...
import { DragArgs, PiTool, normalizeCoordinate } from "../types.js";
...
export const dragTool: PiTool<DragArgs> = {
  ...
  parameters: {
    ...
    properties: {
      ...
      modifiers: {
        ...
        description:
          "Optional keyboard modifier keys to hold down during the drag (e.g. ['alt'], ['shift'], ['ctrl']). Useful for Alt-drag duplicating clips or constraint dragging.",
      },
    },
  ...
  execute: async (args: DragArgs) => {
    ...
    if (!args || typeof args !== "object") {
      return client.formatErrorResponse(
        new Error("Invalid arguments: expected an object with x1, y1, x2, y2 coordinates.")
      );
    }
    ...
    for (const [name, val] of [
      ...
      if (typeof val !== "number" || isNaN(val) || !isFinite(val) || val < 0.0 || val > 1000.0) {
        return client.formatErrorResponse(
          new Error(
            `Invalid '${name}' coordinate: must be a finite number between 0 and 1000 (or 0.0 and 1.0). Received: ${val}`
          )
        );
      }
    }
    ...
    try {
      ...
      return client.formatScreenshotResponse(
        res,
        `Dragged mouse${modStr} from (${normX1.toFixed(4)}, ${normY1.toFixed(4)}) to (${normX2.toFixed(
          4
        )}, ${normY2.toFixed(4)}).`
      );
    } catch (err) {
