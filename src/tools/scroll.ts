...
import { PiTool, ScrollArgs, normalizeCoordinate } from "../types.js";

export const scrollTool: PiTool<ScrollArgs> = {
  name: "scroll",
  description:
    "[Requires active CUA session started via 'start_session'] Positions the mouse at coordinates (x, y) and scrolls in the specified direction ('up', 'down', 'left', or 'right') by the specified amount (default: 3). Supports standard [0, 1000] integer scale or [0.0, 1.0] unit scale. Returns a screenshot.",
  parameters: {
    type: "object",
    properties: {
      x: {
  ...
  execute: async (args: ScrollArgs) => {
    ...
    if (!args || typeof args !== "object") {
      return client.formatErrorResponse(
        new Error("Invalid arguments: expected an object with x, y, and direction.")
      );
    }
    ...
    try {
      ...
      return client.formatScreenshotResponse(
        res,
        `Scrolled ${normalizedDir} by ${scrollAmount} steps at (${normX.toFixed(4)}, ${normY.toFixed(
          4
        )}).`
      );
    } catch (err) {
