...
export const moveMouseTool: PiTool<MoveMouseArgs> = {
  name: "move_mouse",
  description:
    "[Requires active CUA session started via 'start_session'] Moves the mouse cursor to normalized (x, y) coordinates (0.0 to 1.0) on the screen and returns a screenshot showing the cursor's new position for visual verification.",
  parameters: {
    type: "object",
    properties: {
      x: {
  ...
  execute: async (args: MoveMouseArgs) => {
    ...
    if (typeof x !== "number" || isNaN(x) || !isFinite(x) || x < 0.0 || x > 1.0) {
      return client.formatErrorResponse(
        new Error(`Invalid 'x' coordinate: must be a finite number between 0.0 and 1.0. Received: ${x}`)
      );
    }

    if (typeof y !== "number" || isNaN(y) || !isFinite(y) || y < 0.0 || y > 1.0) {
      return client.formatErrorResponse(
        new Error(`Invalid 'y' coordinate: must be a finite number between 0.0 and 1.0. Received: ${y}`)
      );
    }
    ...
    try {
      ...
      return client.formatScreenshotResponse(
        res,
        `Cursor moved to normalized position (${x.toFixed(4)}, ${y.toFixed(4)}).`
      );
    } catch (err) {
