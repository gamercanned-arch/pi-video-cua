...
export const typeTextTool: PiTool<TypeTextArgs> = {
  ...
  execute: async (args: TypeTextArgs) => {
    ...
    if (!args || typeof args !== "object") {
      return client.formatErrorResponse(
        new Error("Invalid arguments: expected an object with 'text' property.")
      );
    }

    if (typeof args.text !== "string") {
      return client.formatErrorResponse(
        new Error(`Invalid 'text' argument: must be a string. Received: ${typeof args.text}`)
      );
    }
    ...
    try {
      ...
      return client.formatScreenshotResponse(
        res,
        `Typed text (${args.text.length} characters) at active focus.`
      );
    } catch (err) {
