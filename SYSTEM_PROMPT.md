# Computer-Use Agent (CUA) System Prompt & Accessibility Guide

This document contains the standard operational system prompt and guidance injected into AI agents (or added to prompt templates) when using the `pi-video-cua` extension.
...
- Never compute or pass raw physical pixel values (e.g., 1920, 1080). Always estimate visually as fractions between 0.0 and 1.0.
...
## 🤖 How Pi Agents Access This Information

When the extension is loaded by `pi`:
1. **Tool Descriptions**: Every tool exposes self-contained parameter schemas, constraints (`minimum: 0.0`, `maximum: 1.0`), and usage docstrings directly in the LLM function-calling JSON schema.
2. **Programmatic System Prompt**: Exported as `CUA_SYSTEM_PROMPT` in `pi-video-cua` so agent runners can automatically prepend or append it to system prompts.
3. **Session Extension Hook**: The `register(piContext)` hook attaches `systemPrompt` and `prompt` metadata into the Pi environment context.
