...
import { HelperClient, startSessionTool, endSessionTool, screenshotTool, moveMouseTool, clickTool, dragTool, waitTool, scrollTool, screenRecordTool, pressKeyTool, typeTextTool, } from "../src/index.js";
...
async function runLiveActualTestSuite() {
    ...
    try {
        ...
        if (!lockedRes.isError)
            throw new Error("Expected move_mouse to be locked before start_session");
        ...
        if (startRes.isError)
            throw new Error("Failed to start session: " + JSON.stringify(startRes));
        const initScreenPath = startRes.details?.imagePath;
        ...
        if (typeRes1.isError)
            throw new Error("Typing text failed: " + JSON.stringify(typeRes1));
        ...
        if (enterRes.isError)
            throw new Error("Press enter failed: " + JSON.stringify(enterRes));
        ...
        if (typeRes2.isError)
            throw new Error("Typing second phrase failed: " + JSON.stringify(typeRes2));
        const afterTypingPath = typeRes2.details?.imagePath;
        ...
        const afterDragPath = dragRes.details?.imagePath;
        ...
        const videoPath = recordRes.details?.videoPath;
        const videoDuration = recordRes.details?.duration;
        ...
        const finalShotPath = finalShot.details?.imagePath;
        // Close CUA session
        ...
        if (!postLockedRes.isError)
            throw new Error("Expected move_mouse to be locked after end_session");
