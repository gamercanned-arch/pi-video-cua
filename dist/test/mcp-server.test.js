...
async function runMcpTest() {
    ...
    try {
        ...
        console.log(` ✓ Successfully listed ${tools.length} tools:`, tools.map((t) => t.name).join(", "));
        ...
        const hasImage = startRes.result?.content?.some((c) => c.type === "image");
        const hasText = startRes.result?.content?.some((c) => c.type === "text");
        if (!hasImage || !hasText) {
