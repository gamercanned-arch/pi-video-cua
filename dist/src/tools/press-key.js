...
export const pressKeyTool = {
    ...
    parameters: {
        ...
        properties: {
            key: {
                type: "string",
                description: "Key name or '+' separated combination, e.g. 'enter', 'space', 'backspace', 'ctrl+s', 'ctrl+z', 'ctrl+shift+z', 'alt+tab', 'f9'.",
            },
        },
        required: ["key"],
    },
    execute: async (args) => {
        ...
        if (typeof args.key !== "string" || args.key.trim().length === 0) {
            return client.formatErrorResponse(new Error(`Invalid 'key' argument: must be a non-empty string. Received: ${JSON.stringify(args?.key)}`));
        }
        try {
