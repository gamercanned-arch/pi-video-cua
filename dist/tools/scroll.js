...
export const scrollTool = {
    ...
    execute: async (args) => {
        ...
        if (!args || typeof args !== "object") {
            return client.formatErrorResponse(new Error("Invalid arguments: expected an object with 'direction' (e.g. { direction: 'up' })."));
        }
        ...
        const hasCoordinates = args.x !== undefined ||
            args.y !== undefined ||
        ...
        if (hasCoordinates) {
            ...
            if (!pt) {
                return client.formatErrorResponse(new Error(`Invalid scroll coordinates: expected valid x and y or pixel coordinates. Received: ${JSON.stringify(args)}`));
            }
            targetPoint = pt;
        }
        ...
        try {
            ...
            return client.formatScreenshotResponse(res, `Scrolled ${normalizedDir} by ${scrollAmount} steps${locStr}.`);
        }
        catch (err) {
